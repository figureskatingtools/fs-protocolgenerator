...
# Source nvm.sh if it exists, but don't fail the script if it returns non-zero (e.g. if no default alias)
if [ -s "$NVM_DIR/nvm.sh" ]; then
    . "$NVM_DIR/nvm.sh" || true
fi

# If nvm command is available, use it. Otherwise assume node is available.
...
echo "Fetching Web App info..."
echo "Using Resource Group: $RESOURCE_GROUP"
APPS_OUTPUT=$(az webapp list --resource-group "$RESOURCE_GROUP" --query "[?contains(name, 'app-fs-protocols')].{name:name, rg:resourceGroup}" -o tsv)

# Check if emptiness
if [ -z "$APPS_OUTPUT" ]; then
    echo "Error: Could not find any Web App matching 'app-fs-protocols'. Make sure you are logged into Azure CLI."
    exit 1
fi
...
# Use the selected RG (override global if needed, but it comes from the app itself)
...
FUNC_INFO=$(az functionapp list --resource-group "$RESOURCE_GROUP" --query "[?contains(name, 'func-fs-protocols')].{name:name, host:defaultHostName}" -o tsv | head -n 1)
...
# 4. Prepare Deployment Artifact
# Structure: server.js + public/ (Vite output)
